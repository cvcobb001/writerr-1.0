...
var addDecorationEffect = StateEffect.define();
var removeDecorationEffect = StateEffect.define();
...
var editDecorationField = StateField.define({
  create() {
  ...
  update(decorations2, tr) {
    ...
    const initialSize = decorations2.size;
    ...
    const finalSize = decorations2.size;
...
var changeDetectionPlugin = ViewPlugin.fromClass(class {
  constructor(view) {
    this.view = view;
  }
  update(update) {
  ...
  extractEditsFromUpdate(update) {
    const edits = [];
    update.changes.iterChanges((from, to, fromB, toB, insert2) => {
      const removedText = update.startState.doc.sliceString(from, to);
      const insertedText = insert2.toString();
      if (removedText) {
        edits.push({
          id: generateId(),
          type: "delete",
...
var TrackEditsPlugin = class extends import_obsidian4.Plugin {
  ...
  extractChangesFromEditor(editor) {
    ...
    const line = doc2.getLine(cursor.line);
    ...
    try {
      ...
      for (const edit of insertionsToRemove) {
        if (edit.text) {
          ...
          const currentText = doc2.sliceString(edit.from, edit.from + edit.text.length);
    ...
    clusterIds.forEach((clusterId) => {
      ...
      if (cluster && this.currentEditorView) {
        ...
        if (insertionsToRemove.length > 0 || deletionsToRestore.length > 0) {
          const doc2 = this.currentEditorView.state.doc;
          const changes = [];
          for (const edit of insertionsToRemove) {
            if (edit.text) {
              const currentText = doc2.sliceString(edit.from, edit.from + edit.text.length);
